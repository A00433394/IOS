...
class gameViewController: UIViewController {
    ...
    @objc func textFeildDidChange(textField:UITextField){
        ...
        if let num_text = numberFeild?.text,
        let in_text = inputFeild?.text,
        let numbers = Int(num_text),
            let input = Int(in_text){
            print("Comparing: \(in_text) minus \(num_text) == \(input -  numbers)")
